...
plt.rcParams['font.sans-serif'] = ['Microsoft YaHei', 'Noto Sans CJK SC', 'Source Han Sans SC', 'SimHei', 'Arial', 'DejaVu Sans', 'Liberation Sans']
...
```python
def apply_publication_style(font_size=13, axes_linewidth=0.95, use_tex=False):
...
**Presets:**
- Chinese manuscript/report figures: `apply_publication_style(font_size=13, axes_linewidth=0.95)`
- Dense Chinese grids: `apply_publication_style(font_size=12, axes_linewidth=0.9)`
- Exact English journal-final dense multi-panels: `apply_publication_style(font_size=8, axes_linewidth=0.8)` only after confirming final physical size and readability
- Large slide-style comparison panels: `apply_publication_style(font_size=18-24, axes_linewidth=2-3)`
- LaTeX labels: `apply_publication_style(use_tex=True)`

For Chinese text, open `typography-layout-contract.md` and use its `choose_cjk_font()`
pattern instead of relying on matplotlib's automatic fallback. Keep one figure to
three font sizes when possible and four at most for dense multi-panel figures.
