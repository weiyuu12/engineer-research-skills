...
|---------|-----------|---------------|
| Chinese manuscript/report figure inserted into Word/PDF | Use the `11 / 13 / 15 / 17` ladder; simple figures usually only `13 / 15 / 17` | 0.9–1.1 |
| Dense Chinese multi-panel grid | Use `11 / 13 / 15 / 17`, or reduce the whole ladder to `10 / 12 / 14 / 16` after preview QA | 0.8–1.0 |
| English journal-final dense multi-panel figure at exact publication width | 7–9 | 0.8–1.2 |
...
When targeting exact final dimensions of a two-column English `Nature` figure page, small
`7–9 pt` final text can be acceptable. Do not use that as the first draft for Chinese
manuscript/report figures or figures that will be inserted into Word/PDF; use
`typography-layout-contract.md` readable defaults first and downscale only after preview QA.
...
- For Chinese figures, reserve enough top/bottom space for larger labels and legends before
  saving. If equal-aspect panels create broad empty corridors, reduce canvas width or adjust
  `wspace`/GridSpec rather than accepting a blank-heavy figure.
...
- [ ] Visual job is explicit; bar/line/heatmap hero panels are used only when metric comparison is truly the central claim
- [ ] Font ladder follows target: Chinese manuscript/report starts around base 12–14 with axis labels 15–16; exact English journal-final may use 7–9 only after preview QA
- [ ] Chinese font family is locked and reused across all figures in the manuscript
...
- [ ] Manual margins or GridSpec reserve space for Chinese labels, group titles, legends, and shared axes; do not rely on `tight_layout()` alone
