...
theme_nature_contract <- function(base_size = 13, base_family = "Microsoft YaHei") {
...
For exact English journal-final assembly, `base_size = 6.5-8` can be used only after
the target physical size is known and the preview remains readable. For Chinese
manuscript/report figures, keep the readable default above and follow
`typography-layout-contract.md`. Keep one figure to three font sizes when possible
and four at most for dense multi-panel figures.
