...
materials for the Python track only. Keep the normal `engineer-figure` contract first:
...
digital-twin, or inverse-design figures, use `engineering-reference-atlas.md` for visual
structure first. Use these figures4papers demos mainly for reusable matplotlib mechanics:
GridSpec layout, axis styling, legend handling, palettes, annotations, and SVG/PDF/TIFF
export conventions.
...
For engineering-domain visual inspiration, prefer
`../assets/engineering-reference-atlas/` through `engineering-reference-atlas.md`.
...
guidance is folded into `engineer-figure`:
